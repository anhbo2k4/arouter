<div align="center">
  ...
  **OpenClaw 的免费 AI 提供商。**
  ...
  [🚀 快速开始](#-quick-start) • [💡 特性](#-key-features) • [📖 设置](#-setup) • [🌐 网站](https://arouter.com)
</div>
...
此仓库包是私有的（`arouter-app`），因此源码/Docker 执行是预期的本地开发路径。
...
默认 URL：
- 仪表板：`http://localhost:1508/dashboard`
- OpenAI 兼容 API：`http://localhost:1508/v1`
...
- 您的 CLI 工具发送 OpenAI 格式 → Arouter 转换 → 提供商接收原生格式
...
> Arouter 本身**从不向您收费**。您只需直接向提供商付款（如果使用付费服务）。
> 
...
- **廉价提供商**（GLM, MiniMax）：直接向他们付款，Arouter 只是路由
...
仪表板追踪您的 Token 使用情况，并显示**估算成本**，就像您直接使用付费 API 一样。这**不是实际计费** - 它是一个参考，显示您通过 Arouter 使用免费模型或现有订阅节省了多少。
...
<details>
<summary><b>💳 Arouter 会向我收费吗？</b></summary>

**不会。** 9 是免费的开源软件，在您自己的计算机上运行。它从不向您收费。

**您只需支付：**
- ✅ **订阅提供商**（Claude Code $20/月, Codex $20-200/月）→ 在他们的网站上直接向他们付款
- ✅ **廉价提供商**（GLM, MiniMax）→ 直接向他们付款，Arouter 只是路由您的请求
- ❌ **Arouter 本身** → **从不收取任何费用，永远**

Arouter 是本地代理/路由器。它没有您的信用卡，不能发送发票，也没有计费系统。完全免费的软件。
...
Router 只是将您的请求路由到它们 - 没有“陷阱”或未来计费。它们是真正的免费服务，Arouter 使它们易于使用并支持回退。

**注意：** 一些订阅提供商（Antigravity, GitHub Copilot）可能有免费预览期，后来可能变成付费，但这会由这些提供商明确宣布，而不是 Arouter。
...
**您在控制中：** 在仪表板中设置每个提供商的支出限制，Arouter 会遵守它们。
...
**专业提示：** 使用 Opus 处理复杂任务，Sonnet 追求速度。Arouter 追踪每个模型的配额！
...
> **注意：** OpenClaw 仅适用于本地 Arouter。使用 `127.0.0.1` 而不是 `localhost` 以避免 IPv6 解析问题。
...
export NEXT_PUBLIC_BASE_URLhttp://localhost:1508"
...
| 变量 | 默认值 | 描述 |
|----------|---------|-------------|
| `JWT_SECRET` | `arouter-default-secret-change-me` | 仪表板认证 cookie 的 JWT 签名密钥（**生产环境中请更改**） |
| `INITIAL_PASSWORD | `123456` | 当没有保存的哈希时的首次登录密码 |
| `DATA_DIR` | `~/.arouter` | 主应用数据库位置（`db.json`） |
| `PORT` | 框架默认值 | 服务端口（示例中为 `1508`） |
| `HOSTNAME` | 框架默认值 | 绑定主机（Docker 默认为 `0.0.0.0`） |
| `NODE_ENV` | 运行时默认值 | 部署时设置 `production` |
| `BASE_URL` |http://localhost:1508` | 云同步作业使用的服务器端内部基础 URL |
| `CLOUD_URL` | `https://arouter.com` | 服务器端云同步端点基础 URL |
| `NEXT_PUBLIC_BASE_URL` | `http://localhost:3000` | 向后兼容/公共基础 URL（服务器运行时优先使用 `BASE_URL`） |
| `NEXT_PUBLIC_CLOUD_URL` | `https://arouter.com` | 向后兼容/公共云 URL（服务器运行时优先使用 `CLOUD_URL`） |
...
- 使用历史和日志：`~/.arouter/usage.json` 和 `~/.arouter/log.txt`，由 `src/lib/usageDb.js` 管理。
...
**仪表板在错误的端口打开**
- 设置 `PORT=1508` 和 `NEXT_PUBLIC_BASE_URL=http://localhost:1508`

**云端同步错误**
- 验证 `BASE_URL` 指向您正在运行的实例（例如：`http://localhost:1508`）
- 验证 `CLOUD_URL` 指向您预期的云端端点（例如：`https://arouter.com`）
- 尽可能保持 `NEXT_PUBLIC_*` 值与服务器端值一致。

**云端端点 `stream=false` 返回 500（`Unexpected token 'd'...`）**
- 症状通常出现在公共云端端点（`https://arouter.com/v1`）的非流式调用上。
- 根本原因：上游返回 SSE 负载（`data: ...`）而客户端期望 JSON。
- 变通方法：对云端直接调用使用 `stream=true`。
- 当上游返回 `text/event-stream` 时，本地 Arouter 运行时包含 SSE→JSON 回退用于非流式调用。
...
```bash
POST httplocalhost:1508/v1/chat/completions
...
- `tester/security/test-cloud-openai-compatible.sh`
  - 使用提供的模型/密钥向云端端点（`https://arouter.com/v1/chat/completions`）发送直接的 OpenAI 兼容请求。
...
- 本地运行时（`http://127.0.0.1:1508/v1/chat/completions`）：使用 `stream=false` 和 `stream=true` 都可以工作。
- Docker 运行时（容器暴露的相同 API 路径）：加固检查通过，云端认证保护工作，启用时严格 API 密钥模式工作。
- 公共云端端点（`https://arouter.com/v1/chat/completions`）：
...
- **网站**：[arouter.com](https://arouter.com)
- **GitHub**：[github.com/decolua/arouter](https://github.com/decolua/arouter)
- **问题**：[github.com/decolua/arouter/issues](https://github.com/decolua/arouter/issues)

---

## 👥 贡献者

感谢所有帮助让 Arouter 变得更好的贡献者！

[![Contributors](https://contrib.rocks/image?repo=decolua/router&max=100&columns=20&anon=1)](https://github.com/decolua/arouter/graphs/contributors)
...
**[OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — Arouter 的全功能 TypeScript 分支。添加了 36+ 提供商、4 层自动回退、多模态 API（图像、嵌入、音频、TTS）、熔断器、语义缓存、LLM 评估和精美的仪表板。8+ 单元测试。通过 npm 和 Docker 可用。
