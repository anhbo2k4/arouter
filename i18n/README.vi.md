...
<div align="center">
  <img src="../images/arouter.png?1" alt="Bảng điều khiển Arouter" width="800"/>
  
  # Arouter - Free AI Router
  ...
  **Nhà cung cấp AI Miễn cho OpenClaw.**
  ...
  [![npm](https://img.shields.io/npm/v/arouter.svg)](https://www.npmjs.com/package/arouter)
  [![Downloads](https://img.shields.io/npm/dm/arouter.svg)](https://www.npmjs.com/package/arouter)
  [![License](https://img.shields.io/npm/l/arouter.svg)](https://github.com/decolua/arouter/blob/main/LICENSE)
  
  [🚀 Bắt đầu nhanh](#-quick-start) • [💡 Tính năng](#-key-features) • [📖 Cài đặt](#-setup-guide) • [🌐 Website](https://arouter.com)
</div>

---

## 🤔 Tại sao chọn Arouter?
...
│   Tool      │
└──────┬──────┘
       │ http://localhost:1508/v1
       ↓
┌────────────────────────────────────────┐
│           Arouter (Smart Router)        │
...
🎉 Bảng điều khiển mở tại `http://localhost:1508`
...
Cài đặt Claude Code/Codex/Gemini CLI/OpenClaw/Cursor/Cline:
  Endpoint: http://localhost:1508/v1
...
Gói kho lưu trữ này là riêng tư (`arouter-app`), vì vậy việc thực thi nguồn/Docker là đường dẫn phát triển cục bộ dự kiến.
...
URL mặc định:
- Bảng điều khiển: `http://localhost:1508/dashboard`
- API tương thích OpenAI: `http://localhost:1508/v1`
...
### 📺 Hướng dẫn thiết lập hoàn chỉnh - Arouter + Claude Code MIỄN PHÍ
  
[![Thiết lập Arouter + Claude Code](https://img.youtube.com/vi/raEyZPg5xE0/maxresdefault.jpg)](https://www.youtube.com/watch?v=raEyZPg5xE0)

**🎬 Xem hướng dẫn từng đầy đủ:**
- ✅ Cài đặt & thiết lập Arouter
...
Arouter hoạt động liền mạch với tất cả các công cụ code AI chính:
...
- Công cụ CLI của bạn gửi định dạng OpenAI → Arouter dịch chuyển → Nhà cung cấp nhận định dạng gốc
...
> Bản thân Arouter **không bao giờ thu phí** bạn bất cứ thứ gì. Bạn chỉ trả tiền trực tiếp cho các nhà cung cấp (nếu sử dụng dịch vụ trả phí).
> 
...
### 📊 Hiểu về Chi phí & Thanh toán của Arouter

**Thực tế Thanh toán Arouter:**

✅ **Phần mềm Arouter = MIỄN PHÍ mãi mãi** (mã nguồn mở, không bao giờ thu phí)
...
❌ **Arouter không bao giờ gửi hóa đơn** hoặc tính phí thẻ của bạn
...
- **Nhà cung cấp giá rẻ** (GLM, MiniMax): Trả tiền trực tiếp cho họ, Arouter chỉ định tuyến
...
<details>
<summary><b>💳 Tôi có bị Arouter tính phí không?</b></summary>

**Không.** Arouter là phần mềm miễn phí, mã nguồn mở chạy trên máy tính của chính bạn. Nó không bao giờ tính phí bạn bất cứ thứ gì.

**Bạn chỉ trả tiền:**
- ✅ **Nhà cung cấp gói đăng ký** (Claude Code $20/tháng, Codex $20-200/tháng) → Trả tiền trực tiếp cho họ trên website của họ
- ✅ **Nhà cung cấp giá rẻ** (GLM, MiniMax) → Trả tiền trực tiếp cho họ, Arouter chỉ định tuyến yêu cầu của bạn
- ❌ **Bản thân Arouter** → **Không bao giờ tính phí bất cứ thứ gì, ever**

Arouter là một proxy/router cục bộ. Nó không cóẻ tín dụng của bạn, không thể gửi hóa đơn và không có hệ thống thanh toán. Đó là phần mềm hoàn toàn miễn phí.
...
Arouter chỉ định tuyến yêu cầu của bạn đến họ - không có "cạm bẫy" hay thanh toán trong tương lai. Đó là các dịch vụ thực sự miễn phí, và Arouter giúp chúng dễ sử dụng với hỗ trợ dự phòng.

**Lưu ý:** số nhà cung cấp gói đăng ký (Antigravity, GitHub Copilot) có thể có các khoảng thời gian dùng thử miễn phí có thể trở thành trả phí sau này, nhưng điều này sẽ được các nhà cung cấp đó thông báo rõ ràng, không phải Arouter.
...
3. **Sử dụng nhà cung cấp gói đăng ký cuối cùng:**
   ...
   - Arouter giúp tối đa hóa giá trị của chúng thông qua theo dõi hạn mức
...
Cơ chế dự phòng thông minh của Arouter ngăn chặn các khoản phí bất ngờ:
...
**Bạn nắm quyền kiểm soát:** Đặt giới hạn chi tiêu cho mỗi nhà cung cấp trong bảng điều khiển, và Arouter sẽ tuân thủ chúng.
...
**Mẹo Chuyên nghiệp:** Sử dụng Opus cho các tác vụ phức tạp, Sonnet cho tốc độ. Arouter theo dõi hạn mức cho mỗi mô hình!
...
Settings → Models → Advanced:
  OpenAI API Base URL: http://localhost:1508/v1
  OpenAI API Key: [từ bảng điều khiển arouter]
...
```json
{
  "anthropic_api_base": "http://localhost:1508/v1",
  "anthropic_api_key": "your-arouter-api-key"
}
```
...
> **Lưu ý:** OpenClaw chỉ hoạt động với Arouter cục bộ. Sử dụng `127.0.0.1` thay vì `localhost` để tránh các vấn đề phân giải6.
...
```bash
# Clone và cài đặt
git clone https://github.com/decolua/arouter.git
cd arouter
npm install
npm run build
...
export DATA_DIR="/var/lib/arouter"
export PORT="1508"
export HOSTNAME="0.0.0.0"
export NODE_ENV="production"
export NEXT_PUBLIC_BASE_URL="http://localhost:1508"
export NEXT_PUBLIC_CLOUD_URL="https://arouter.com"
...
docker run -d \
  --name arouter \
  -p 1508:1508 \
  --env-file /root/dev/arouter/.env \
  -v arouter-data:/app/data \
  -v arouter-usage:/root/.arouter \
  arouter
```
...
docker logs -f arouter
 restart arouter
docker stop arouter && docker rm arouter
```
...
| Biến | Mặc định | Mô tả |
|----------|---------|-------------|
| `JWT_SECRET` | `arouter-default-secret-change-me` | Bí mật ký JWT cho cookie xác thực bảng điều khiển (**thay đổi trong production**) |
| `INITIAL_PASSWORD` | `123456` | Mật khẩu đăng nhập đầu tiên khi không có hash đã lưu tồn tại |
| `DATA_DIR` | `~/.arouter` |ị trí cơ sở dữ liệu ứng dụng chính (`db.json`) |
| `PORT` | framework default | Cổng dịch vụ (`1508` trong các ví dụ) |
| `HOSTNAME` | framework default | Bind host (Docker mặc định là `0.0.0.0`) |
| `NODE_ENV` | runtime default | Đặt `production` để triển khai |
| `BASE_URL` | `http://localhost:1508` | URL cơ sở nội bộ phía máy chủ được sử dụng bởi các tác vụ đồng bộ đám mây |
| `CLOUD_URL` | `https://arouter.com` | URL cơ sở endpoint đồng bộ đám mây phía máy chủ |
| `NEXT_PUBLIC_BASE_URL` | `http://localhost:3000` | URL cơ sở tương thích ngược/công khai (ưu tiên `BASE_URL` cho runtime máy chủ) |
| `NEXT_PUBLIC_CLOUD_URL` | `https://arouter.com` | URL đám mây tương thích ngược/công khai (ưu tiên `CLOUD_URL` cho runtime máy chủ) |
...
- Lịch sử sử dụng và log: `~/.arouter/usage.json` và `~/.arouter/log.txt`, được quản lý bởi `src/lib/usageDb.js`.
- request/translator tùy chọn: `<repo>/logs/...` khi `ENABLE_REQUEST_LOGS=true`.
- Lưu trữ sử dụng hiện tại tuân theo logic đường dẫn `~/.arouter` và độc lập với `DATA_DIR`.
...
- Đặt `PORT=1508` và `NEXT_PUBLIC_BASE_URL=http://localhost:1508`

**Lỗi đồng bộ đám mây**
- Xác minh `BASE_URL` trỏ đến phiên bản đang chạy của bạn (ví dụ: `http://localhost:1508`)
- Xác minh `CLOUD_URL` trỏ đến endpoint đám mây dự kiến của bạn (ví dụ: `https://arouter.com`)
...
- Triệu chứng thường xuất hiện trên endpoint đám mây công khai (`https://arouter.com/v1`) cho các lệnh gọi không phát trực tiếp (non-streaming).
...
- Runtime Arouter cục bộ bao gồm dự phòng SSE→JSON cho các lệnh gọi không phát trực tiếp khi upstream trả về `text/event-stream`.
...
```bash
POST http://localhost:1508/v1/chat/completions
...
```bash
GET http://localhost:1508/v1/models
Authorization: Bearer your-api-key
...
- `tester/security/test-cloud-openai-compatible.sh`
  - Gửi một yêu cầu tương thích OpenAI trực tiếp đến endpoint đám mây (`https://arouter.com/v1/chat/completions`) với mô hình/key được cung cấp.
...
- cục bộ (`http://127.0.0.1:1508/v1/chat/completions`): hoạt động với `stream=false` và `stream=true`.
...
- Endpoint đám mây công khai (`https://arouter.com/v1/chat/completions`):
...
- **Website**: [arouter.com](https://arouter.com)
- **GitHub**: [github.com/decolua/9](https://github.com/decolua/arouter)
- **Issues**: [github.com/decolua/arouter/issues](https://github.com/decolua/arouter/issues)

---

## 👥 Người đóng góp

Cảm ơn tất cả những người đã đóng góp giúp Arouter tốt hơn!

[![Contributors](https://contrib.rocks/image?repo=decolua/arouter&max=100&columns=20&anon=1)](https://github.com/decolua/arouter/graphs/contributors)

---

## 📊 Star Chart

[![ Chart](https://starchart.cc/decolua/arouter.svg?variant=adaptive)](https://starchart.cc/decolua/arouter)
