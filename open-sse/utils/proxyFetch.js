...
function shouldUseAnthropicScraping(targetUrl, options) {
  try {
    const parsed = new URL(targetUrl);
    const accept = new Headers(options.headers || {}).get("accept") || "";
    return parsed.hostname === "api.anthropic.com" && !accept.includes("text/event-stream");
  } catch {
    return false;
  }
}

function gotResponseToFetchResponse(response) {
...
export async function proxyAwareFetch(url, options = {}, proxyOptions = null) {
  ...
  if (shouldUseAnthropicScraping(targetUrl, options)) {
    try {
      ...
      return gotResponseToFetchResponse(response);
    } catch (error) {
      console.warn(`[ProxyFetch] got-scraping Anthropic path failed, falling back to fetch: ${error.message}`);
    }
  }
