Căn bản thì bố mày là nhất

<div align="center">
  <img src="./images/arouter.png?1" alt="Arouter Dashboard" width="800"/>

  # clone_arouter - Personal AI Router Setup
  ...
  **Dành cho nhu cầu cá nhân: quản lý key, quota, provider và các luồng test nhanh không cần CLI.**
  ...
  [🚀 Quick Start](#-quick-start) • [💡 Features](#-key-features) • [📖 Setup](#-setup-guide) • [📦 Repo](https://github.com/anhbo2004/clone_arouter)

  [📘 Huong Dan Su Dung (VI)](./HUONG_DAN_SU_DUNG_VI.md)
...
> Note: README này là bản dùng riêng cho repo cá nhân `clone_arouter`, tập trung vào workflow thực tế của tôi.
...
🎉 Dashboard opens at `http://localhost:1508`
...
This repository package is private (`arouter-app`), so source/Docker execution is the expected local development path.
...
What you can do:
- Create key with quota window (`rolling_5h`, `daily`, `weekly`, `monthly`)
- Set total/input/output token limits
- Set allowed models per key
- Enable/Disable/Delete key
- Quick test per key (no CLI)

#### 2. Quick Test (No CLI)

In `API Key Token Limits` table, click:
- `Quick test`

Result:
- Immediate pass/fail message in UI
- Usage is recorded and displayed in the same table

#### 3. Public API Key Usage Checker (No Login)
...
- Quota window + start time
- Used tokens (input/output/total)
- Remaining quota
- Exceeded status

#### 4. Bulk Codex Auto Login (Many Accounts)

Open:
- `Dashboard -> Providers`
- Click `Bulk Codex Auto Login`
...
Notes:
- Uses browser automation (Playwright) + TOTP for 2FA
- Returns per-row result (`OK` / `FAIL`) in modal
- Accounts with extra CAPTCHA/challenge may fail and require manual retry
...
### 📺 Complete Setup Guide - Arouter + Claude Code FREE

[![Arouter + Claude Code Setup](https://img.youtube.com/vi/raEyZPg5xE0/maxresdefault.jpg)](https://www.youtube.com/watch?v=raEyZPg5xE0)

**🎬 Watch the complete step-by-step tutorial:**
- ✅ Arouter installation & setup
...
Arouter works seamlessly with all major AI coding tools:
...
- Your CLI tool sends OpenAI format → Arouter translates → Provider receives native format
...
> Arouter itself **never charges** you anything. You only pay providers directly (if using paid services).
>
...
### 📊 Understanding Arouter Costs & Billing

**Arouter Billing Reality:**

✅ **Arouter software = FREE forever** (open source, never charges)
✅ **Dashboard "costs" = Display/tracking only** (not actual bills)
✅ **You pay providers directly** (subscriptions or API fees)
✅ **FREE providers stay FREE** (iFlow, Kiro, Qwen = $0 unlimited)
❌ **Arouter never sends invoices** or charges your card
...
- **Cheap providers** (GLM, MiniMax): Pay them directly, Arouter just routes
- **FREE providers** (iFlow, Kiro, Qwen): Genuinely free forever, no hidden charges
- **Arouter**: Never charges anything, ever
...
The dashboard tracks your token usage and displays **estimated costs** as if you were using paid APIs directly. This is **not actual billing** - it's a reference to show how much you're saving by using free models or existing subscriptions through Arouter.
...
<details>
<summary><b>💳 Will I be charged by Arouter?</b></summary>

**No.** Arouter is free, open-source software that runs on your own computer. It never charges you anything.

**You only pay:**
- ✅ **Subscription providers** (Claude Code $20/mo, Codex $20-200/mo) → Pay them directly on their websites
- ✅ **Cheap providers** (GLM, MiniMax) → Pay them directly, Arouter just routes your requests
- ❌ **Arouter itself** → **Never charges anything, ever**

Arouter is a local proxy/router. It doesn't have your credit card, can't send invoices, and has no billing system. It's completely free software.
...
Arouter just routes your requests to them - there's no "catch" or future billing. They're truly free services, and Arouter makes them easy to use with fallback support.
...
3. **Use subscription providers last:**
   ...
   - Arouter helps maximize their value through quota tracking
...
Arouter's smart fallback prevents surprise charges:
...
**You're in control:** Set spending limits per provider in dashboard, and Arouter respects them.
...
**Pro Tip:** Use Opus for complex tasks, Sonnet for speed. Arouter tracks quota per model!
...
Settings → Models → Advanced:
  ...
  OpenAI API Key: [from arouter dashboard]
...
```json
{
  "agents": {
    "defaults": {
      "model": {
        "primary": "arouter/kr/claude-sonnet-4.5"
      }
    }
  },
  "models": {
    "providers": {
      "arouter": {
        "baseUrl": "http://127.0.0.1:1508/v1",
        "apiKey": "sk_arouter",
...
> **Note:** OpenClaw only works with local Arouter. Use `127.0.0.1` instead of `localhost` to avoid IPv6 resolution issues.
...
# Or use PM2
npm install -g pm2
pm2 start npm --name arouter -- start
pm2 save
pm2 startup
```
...
|----------|---------|-------------|
| `JWT_SECRET` | `arouter-default-secret-change-me` | JWT signing secret for dashboard auth cookie (**change in production**) |
| `INITIAL_PASSWORD` | `123456` | First login password when no saved hash exists |
| `DATA_DIR` | `~/.arouter` | Main app database location (`db.json`) |
| `PORT` | framework default | Service port (`1508` in examples) |
| `HOSTNAME` | framework default | Bind host (Docker defaults to `0.0.0.0`) |
| `NODE_ENV` | runtime default | Set `production` for deploy |
| `BASE_URL` | `http://localhost:1508` | Server-side internal base URL used by cloud sync jobs |
| `CLOUD_URL` | `https://arouter.com` | Server-side cloud sync endpoint base URL |
| `NEXT_PUBLIC_BASE_URL` | `http://localhost:3000` | Backward-compatible/public base URL (prefer `BASE_URL` for server runtime) |
| `NEXT_PUBLIC_CLOUD_URL` | `https://arouter.com` | Backward-compatible/public cloud URL (prefer `CLOUD_URL` for server runtime) |
...
- Usage history and logs: `~/.arouter/usage.json` and `~/.arouter/log.txt`, managed by `src/lib/usageDb.js`.
- Optional request/translator logs: `<repo>/logs/...` when `ENABLE_REQUEST_LOGS=true`.
- Usage storage currently follows `~/.arouter` path logic and is independent from `DATA_DIR`.
...
- Set `PORT=1508` and `NEXT_PUBLIC_BASE_URL=http://localhost:1508`
...
- **GitHub**: [github.com/anhbo2004/clone_arouter](https://github.com/anhbo2004/clone_arouter)
- **Issues**: [github.com/anhbo2004/clone_arouter/issues](https://github.com/anhbo2004/clone_arouter/issues)
