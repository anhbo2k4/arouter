...
Arouter is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[Arouter Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.arouter/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.arouter/usage.json`, `~/.arouter/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.arouter/db.json`)
- usage stats: `~/.arouter/usage.json`
- request log lines: `~/.arouter/log.txt`
...
flowchart LR
    ...
    subgraph ContainerOrProcess[Arouter Runtime]
        Next[Next.js Server\nPORT=1508]
...
1. `usageDb` currently stores under `~/.arouter` and does not follow `DATA_DIR`.
...
- Build from source: `cd /root/dev/arouter && npm run build`
- Build Docker image: `cd /root/dev/arouter && docker build -t arouter .`
- Start service and verify:
- `GET /api/settings`
- `GET /api/v1/models`
- CLI target base URL should be `http://<host>:1508/v1` when `PORT=1508`
