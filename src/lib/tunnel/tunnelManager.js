...
import { buildTunnelApiUrl, buildTunnelPublicUrl } from "./tunnelUrls.js";
...
export async function enableTunnel(localPort = 1508) {
  manualDisabled = false;
  activeLocalPort = localPort;

  if (isCloudflaredRunning(localPort)) {
    const existing = loadState();
    if (existing?.tunnelUrl) {
      const publicUrl = buildTunnelPublicUrl(existing.shortId);
      const apiUrl = buildTunnelApiUrl(existing.tunnelUrl);
      return { success: true, tunnelUrl: existing.tunnelUrl, apiUrl, shortId: existing.shortId, publicUrl, alreadyRunning: true };
    }
  }
  ...
  const publicUrl = buildTunnelPublicUrl(shortId);
  const apiUrl = buildTunnelApiUrl(tunnelUrl);
  return { success: true, tunnelUrl, apiUrl, shortId, publicUrl };
}
...
export async function getTunnelStatus() {
  const state = loadState();
  const running = isCloudflaredRunning(activeLocalPort || 1508);
  const settings = await getSettings();
  const shortId = state?.shortId || "";
  const publicUrl = buildTunnelPublicUrl(shortId);
  const apiUrl = buildTunnelApiUrl(state?.tunnelUrl || "");
...
export async function enableTailscale(localPort = 1508) {
