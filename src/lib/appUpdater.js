  ...
  try {
    const mitmPidFile = path.join(
      process.platform === "win32"
        ? path.join(process.env.APPDATA || "", "arouter")
        : path.join(os.homedir(), ".arouter"),
      "mitm",
      ".mitm.pid"
    );
...
// Collect PIDs of all arouter-related processes (excluding current)
function collectAppPids() {
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        const isAppProcess = line.toLowerCase().includes("arouter") || line.toLowerCase().includes("next-server");
        if (isAppProcess) {
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("arouter") || line.includes("next-server") || line.includes("cloudflared");
        if (isAppProcess) {
...
// Resolve npx/arouter binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx arouter` — works regardless of global bin path changes after npm i -g
