...
const hasArouterConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "Arouter");
};

const getArouterEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "Arouter") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getArouterEntry(config);
...
// POST - Apply Arouter config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing Arouter entry or append
    const idx = config.findIndex((e) => e.name === "Arouter");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove Arouter entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "Arouter");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "Arouter removed from Copilot config",
    });
  } catch (error) {
