...
// Check if settings has Arouter customModels
const hasArouterConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:Arouter"));
};
...
// POST - Update Arouter customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing Arouter configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:Arouter"));
...
// DELETE - Remove Arouter customModels only (keep other settings)
