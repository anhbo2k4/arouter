...
const hasArouterConfig = (config) => {
  if (!config?.provider) return false;
  return !!config.provider["arouter"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["arouter"];
    const modelMap = providerConfig?.models || {};

    return NextResponse.json({
      installed: true,
      config,
      hasArouter: hasArouterConfig(config),
      configPath: getConfigPath(),
        opencode: {
          models: Object.keys(modelMap),
          activeModel: config?.model?.startsWith("arouter/") ? config.model.replace(/^arouter\//, "") : null,
...
// POST - Apply Arouter as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_arouter";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing arouter provider entry and its models
    const existingProvider = config.provider["arouter"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["arouter"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `arouter/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `arouter/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("arouter/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove Arouter provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["arouter"]?.models) {
      delete config.provider["arouter"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["arouter"].models).length === 0) {
        delete config.provider["arouter"];
        if (config.model?.startsWith("arouter/")) delete config.model;
      } else if (config.model === `arouter/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["arouter"].models);
        config.model = `arouter/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire arouter provider
      if (config.provider) delete config.provider["arouter"];
      if (config.model?.startsWith("arouter/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("arouter/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "Arouter settings removed from OpenCode",
    });
  } catch (error) {
