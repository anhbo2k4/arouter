...
export default function DroidToolCard({
  ...
  const getConfigStatus = () => {
    ...
    // Check for any Arouter model entry (support multi-model: custom:Arouter-0, custom:Arouter-1, ...)
    const currentConfig = droidStatus.settings?.customModels?.find(m => m.id?.startsWith("custom:Arouter"));
  ...
  useEffect(() => {
    if (droidStatus?.installed && !hasInitializedModel.current) {
      ...
      const existingModels = (droidStatus.settings?.customModels || [])
        .filter(m => m.id?.startsWith("custom:Arouter"))
        .sort((a, b) => (a.index || 0) - (b.index || 0))
        .map(m => m.model);
      if (existingModels.length > 0) {
        setModelList(existingModels);
      } else {
        // Legacy: single model stored as custom:Arouter-0
        const legacy = droidStatus.settings?.customModels?.find(m => m.id === "custom:Arouter-0");
        if (legacy?.model) {
  ...
  const getManualConfigs = () => {
    const keyToUse = (selectedApiKey && selectedApiKey.trim())
      ? selectedApiKey
      : (!cloudEnabled ? "sk_arouter" : "<API_KEY_FROM_DASHBOARD>");
              ...
              <div className="flex flex-col gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-text-muted">Manual configuration is still available if arouter is deployed on a remote server.</p>
                  </div>
                </div>
          ...
          {!checkingDroid && droidStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                {/* Current Base URL */}
                {droidStatus?.settings?.customModels?.find(m => m.id?.startsWith("custom:Arouter"))?.baseUrl && (
                  <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {apiKeys.length > 0 ? (
                    ...
                    <span className="flex-1 text-xs text-text-muted px-2 py-1.5">
                      {cloudEnabled ? "No API keys - Create one in Keys page" : "sk_arouter (default)"}
                    </span>
                  )}
                </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!droidStatus?.hasArouter} loading={restoring}>
