  ...
  return (
    <>
      ...
      <ConfirmModal
        ...
        title="Update Arouter"
        message={`This will close Arouter and install v${updateInfo?.latestVersion || ""} in a separate window. Continue?`}
        confirmText="Update"
        cancelText="Cancel"
        variant="primary"
        loading={isUpdating}
...
function UpdateProgress({ status, latestVersion, installCmd, copied, onCopy }) {
  ...
  const steps = [
    { key: "stopped", label: "Stopped Arouter server", state: "done" },
    {
      key: "launched",
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h2 className="text-lg font-semibold">
            {done && success ? "Update Completed" : done && !success ? "Update Failed" : "Updating Arouter"}
          </h2>
      ...
      {done && success ? (
        <div className="space-y-2">
          <p className="text-sm text-white/80">
            Run <code className="px-1.5 py-0.5 rounded bg-white/10 text-green-400">arouter</code> in your terminal to start the new version.
          </p>
